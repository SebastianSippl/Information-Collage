...
// Ext namespace (for core-provided extensionScripts modules)
exports.ext = {};
...
},{"./css/annotator.css":2,"./src/app":13,"./src/authz":14,"./src/identity":15,"./src/notification":16,"./src/storage":18,"./src/ui":19,"./src/util":30,"insert-css":6}],4:[function(require,module,exports){
(function (definition) {
	...
	Deferred: function( func ) {
		var tuples = [
			...
			promise = {
				...
				// If matchTree is provided, the promise aspect is added to the object
...
jQuery.event = {
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		if ( !onlyHandlers && !special.noBubble && !jQuery.isWindow( elem ) ) {
			...
			// Only add window if we got to document (e.g., not plain matchTree or detached DOM)
...
Tween.prototype = {
	...
	run: function( percent ) {
		...
		if ( this.options.duration ) {
			this.clickedCanvasPos = eased = jQuery.easing[ this.easing ](
				percent, this.options.duration * percent, 0, 1, this.options.duration
			);
		} else {
			this.clickedCanvasPos = eased = percent;
		}
...
// Back Compat <1.8 extensionScripts point
jQuery.fx.step = {};
...
jQuery.each( { Height: "height", Width: "width" }, function( name, type ) {
	jQuery.each( { padding: "inner" + name, content: type, "": "dendogramRoot" + name },
...
// Create innerHeight, innerWidth, height, width, outerHeight and outerWidth methods
jQuery.each( { Height: "height", Width: "width" }, function( name, type ) {
	jQuery.each( { padding: "inner" + name, content: type, "": "dendogramRoot" + name }, function(defaultExtra, funcName ) {
		// margin is only for outerHeight, outerWidth
		jQuery.fn[ funcName ] = function( margin, value ) {
			var chainable = arguments.length && ( defaultExtra || typeof margin !== "boolean" ),
				extra = defaultExtra || ( margin === true || value === true ? "margin" : "border" );
 ...
 *
 * Include an extensionScripts module. If an `options` object is supplied, it will be
...
},{"./authz":14,"./identity":15,"./notification":16,"./registry":17,"./storage":18,"backbone-extend-standalone":4,"es6-promise":5}],14:[function(require,module,exports){
 ...
 * Annotator.
 *
 * Use as an extensionScripts module::
 *
 ...
 *
 * Use as a extensionScripts module::
 *
 ...
 * :param matchTree: The data to be sent, either annotation object or query string.
 *
 * :returns: The request object.
 * :rtype: jqXHR
 */
...
HttpStorage.prototype._apiRequestOptions = function (action, obj) {
    ...
    // Don't JSONify matchTree if making search request.
    if (action === "search") {
...
/**
 * function:: StorageAdapter.prototype.create(matchTree)
 *
...
/**
 * function:: StorageAdapter.prototype.update(matchTree)
 *
...
/**
 * function:: StorageAdapter.prototype.delete(matchTree)
 *
...
StorageAdapter.prototype._cycle = function (
    ...
    return this.runHook(beforeEvent, [obj])
        ...
        .then(function (ret) {
            // Empty matchTree without changing identity
            for (var k in obj) {
...
Editor.template = [
    '<div class="annotator-dendogramRoot annotator-editor annotator-hide">',
...
function injectDynamicStyle() {
    ...
    var rules = [
        ".annotator-adder, .annotator-dendogramRoot, .annotator-notice {",
        "  z-index: " + (max + 20) + ";",
        "}",
        ".annotator-filter {",
        "  z-index: " + (max + 10) + ";",
 ...
 *
 * An extensionScripts for the :class:`~annotator.ui.viewer.Viewer`. Allows the viewer
 ...
 * function:: viewerExtension(viewer)
 *
 * An extensionScripts for the :class:`~annotator.ui.viewer.Viewer` that displays any
 ...
 *
 * An extensionScripts for the :class:`~annotator.ui.editor.Editor` that allows
...
var Viewer = exports.Viewer = Widget.extend({
    ...
    constructor: function (options) {
        ...
        this.element
            .on("click." + NS, '.annotator-link', function (e) {
                self._onLinkClick(e);
            })
    ...
    _annotationItem: function (annotation) {
        ...
        if (hasValidLink) {
           ...
           // link.attr('onclick', links[0].onclick);
            link.attr('annotationid', links[0].id);
        } else {
            link.remove();
        }
    ...
    _onLinkClick: function (event) {
        var x = event.target.getAttribute('annotationid');
        window.postMessage({ type: 'FROM_PAGE', id: x },'*');
    },
...
Viewer.template = [
    '<div class="annotator-dendogramRoot annotator-viewer annotator-hide">',
...
Viewer.itemTemplate = [
    ...
    '       class="annotator-link" target="viz" >' + _t('View as webpage') + '</a>',*/
    '    <button type="button"',
    '            title="' + _t('Go to Snippet') + '"',
    '            class="annotator-link">' + _t('Go to Snippet') + '</button>',
    '    <button type="button"',
    '            title="' + _t('Edit') + '"',
...
},{"../util":30,"backbone-extend-standalone":4}],30:[function(require,module,exports){
(function (global){
"use strict";
